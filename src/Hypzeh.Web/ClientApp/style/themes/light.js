import {
  WHITE,
  BLACK,
  GRAY,
  LIGHT_GRAY,
  YELLOW,
  CINNABAR,
  DANGER,
  TRANSPARENT,
} from '../colours';

const COLOURS = {
  // Ordered by component
  // A
  // B
  button_background_colour: YELLOW,
  button_background_colour_disabled: '#d9de17',
  button_background_colour_hover: '#fdf412',
  button_border_colour: BLACK,
  button_border_colour_hover: YELLOW,
  button_colour: BLACK,
  button_colour_disabled: GRAY,
  button_colour_hover: BLACK,
  button_danger_background_colour: TRANSPARENT,
  button_danger_background_colour_disabled: CINNABAR,
  button_danger_background_colour_hover: DANGER,
  button_danger_border_colour: CINNABAR,
  button_danger_border_colour_hover: CINNABAR,
  button_danger_colour: CINNABAR,
  button_danger_colour_disabled: LIGHT_GRAY,
  button_danger_colour_hover: BLACK,
  button_link_colour_highlight: '#7f58b8',
  button_outline_background_colour: TRANSPARENT,
  button_outline_background_colour_disabled: '#d9de17',
  button_outline_background_colour_hover: '#fdf412',
  button_outline_border_colour: BLACK,
  button_outline_border_colour_hover: BLACK,
  button_outline_colour: BLACK,
  button_outline_colour_disabled: GRAY,
  button_outline_colour_hover: BLACK,
  // C
  // D
  // E
  // F
  // G
  global_background_primary: WHITE,
  global_content_primary: BLACK,
  global_danger: DANGER,
  // H
  home_introduction_colour_highlight: '#7f58b8',
  // I
  // J
  // K
  // L
  loader_background_colour: BLACK,
  // M
  // N
  navbar_background_colour: '#e2e2e2',
  navbar_link_background_colour_active: '#f2f2f2',
  navbar_link_background_colour_hover: '#614092',
  navbar_link_border_colour_active: '#7f58b8',
  navbar_link_colour: BLACK,
  navbar_link_colour_active: '#7f58b8',
  navbar_link_colour_hover: WHITE,
  // O
  // P
  // Q
  // R
  // S
  sidebar_background_colour: '#f2f2f2',
  sidebar_link_background_colour_active: '#7f58b8',
  sidebar_link_background_colour_hover: '#614092',
  sidebar_link_colour: BLACK,
  sidebar_separator_colour: '#7f58b8',
  smallify_download_colour_highlight: '#7f58b8',
  splash_screen_background_colour: WHITE,
  splash_screen_colour: BLACK,
  splash_screen_status_error_colour: DANGER,
  splash_screen_status_standard_colour: BLACK,
  splash_screen_status_warning_colour: DANGER,
  // T
  terminal_input_colour: BLACK,
  terminal_output_colour_highlight: '#7f58b8',
  // U
  // V
  // W
  // X
  // Y
  // Z
};

export default {
  colours: COLOURS,
};
